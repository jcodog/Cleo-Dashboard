...
export const ServerCard = ({
  ...
  const recentClass = isRecent
    ? "ring-4 ring-[var(--color-chart-2)] ring-offset-2 ring-offset-background"
    : "";
  ...
  return (
    <>
      <Link
        ...
        className={`flex md:hidden group relative w-auto aspect-square h-auto overflow-hidden bg-accent/20 shadow-lg rounded-lg cursor-pointer z-0 transition-transform hover:-translate-y-0.5 ${recentClass}`}
      >
        {isRecent && (
      ...
      <Link
        href={`/dashboard/guild/${serverId}`}
        className={`hidden md:flex group relative w-full aspect-square h-auto overflow-hidden bg-accent/20 shadow-lg rounded-lg cursor-pointer z-0 transition-transform hover:-translate-y-0.5 ${recentClass}`}
        style={{ maxWidth: 340 }}
      >
        {isRecent && (
          <span className="absolute top-2 left-2 bg-indigo-600 text-white text-xs font-semibold px-2 py-0.5 rounded-md z-20">
