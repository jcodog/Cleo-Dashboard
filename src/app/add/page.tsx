...
const AddPage = () => {
  ...
  const { mutate, data: generatedUrl } = useMutation({
    ...
    mutationFn: async (mode: "user" | "server") => {
      const res = await client.discord.addCleo.$post({ mode }, undefined);
  ...
  return (
    <section className="flex flex-1 flex-col items-center justify-center gap-8 max-w-3xl">
      <div className="flex flex-col items-center justify-center gap-2 text-pretty text-center">
        ...
        <p>
          Cleo is an amazing tool that works as either a server bot or a bot
          that can be used in group DMs with your friends (user install). We
          want to help make the setup of Cleo super enjoyable and easy for you.
        </p>
      </div>
      ...
      <div className="flex flex-col items-center justify-center text-pretty gap-2">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={() => router.push("/add/server")}>
            <Server /> Server
          </Button>
        </div>
      </div>
